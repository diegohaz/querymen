...
test('MenquerySchema', (t) => {
  ...
  t.comment('MenquerySchema add')
  let add = (...args) => schema().add('test', ...args).value()
  t.true(mySchema.add(new Param('test')), 'should not add a param with instance')
  ...
  t.comment('MenquerySchema option')
  ...
  t.comment('MenquerySchema param')
  ...
  t.comment('MenquerySchema formatter')
  ...
  t.comment('MenquerySchema validator')
  ...
  t.comment('MenquerySchema parser')
  ...
  t.same(mySchema.parse().filter, {test: 'HELP'}, 'should not apply parser')
  t.true(mySchema.param('test').option('elemMatch', 'prop'), 'should set param option')
  t.same(mySchema.parse().filter, {test: {$elemMatch: {prop: 'HELP'}}}, 'should apply parser')
  t.true(mySchema.param('p', null).parser('elemMatch'), 'should get lazy param parser')

  mySchema = schema({}, {page: 'p'})

  t.comment('MenqueryParam name')
  ...
  t.comment('MenqueryParam default parse')
  let parse = (...args) => schema().parse(...args)
  t.same(parse({q: 'testing'}).filter._q, /testing/i, 'should parse q')
  t.same(parse({page: 2, limit: 10}).options.skip, 10, 'should parse page')
  t.same(parse({limit: 10}).options.limit, 10, 'should parse limit')
  t.same(parse({sort: '-createdAt'}).options.sort, {createdAt: -1}, 'should parse sort')
  t.same(parse({sort: '-createdAt,name,+test'}).options.sort, {createdAt: -1, name: 1, test: 1}, 'should parse multiple sort')
  t.same(schema({distance: 0}).parse({distance: 23}).filter.distance, 23, 'should parse any')
